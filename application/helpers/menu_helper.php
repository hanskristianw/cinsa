...
function admission_menu()
{
  ...
  $data = $ci->db->query(
    "SELECT *
    FROM kr
    WHERE kr_jabatan_id = 8 AND kr_id = $kr_id"
  )->result_array();
...
function return_mk($mapel_id, $d_s_id)
{
  ...
  $mapel_lain = $ci->db->query(
    ...
    WHERE mk_detail_d_s_id = ' . $d_s_id . ' AND mk_mapel_id = ' . $mapel_id . ''
  )->row_array();
...
function disjam_sekolah_lain($kr_id, $t_id, $sk_id)
{
  ...
  $sk_lain = $ci->db->query(
    ...
    WHERE d_mpl_kr_id = ' . $kr_id . ' AND kelas_t_id = ' . $t_id . ' AND kelas_sk_id <>  ' . $sk_id . ''
  )->result_array();
...
function return_konseling_report($d_s_id)
{
  ...
  $raport_konseling = $ci->db->query(
    ...
    WHERE konseling_d_s_id = ' . $d_s_id . ' ORDER BY konseling_tanggal'
  )->result_array();
...
function return_raport_mid($d_s_id, $semester)
{
  ...
  $raport_mid = $ci->db->query(
    'SELECT * FROM
      ...
      )as mk_fix ON formative.mapel_id = mk_fix.mk_mapel_id'
  )->result_array();
...
function show_af_count($k_afek_id, $mapel_id, $kelas_id)
{
  ...
  $laporan = $ci->db->query(
    ...
    WHERE afektif_k_afek_id = $k_afek_id AND afektif_mapel_id= $mapel_id AND d_s_kelas_id = $kelas_id"
  )->row_array();
...
function return_raport_fin($d_s_id, $semester, $jenjang, $t_id)
{
  ...
  ORDER BY formative.mapel_urutan"
  )->result_array();
...
function returnGuruSsp($d_s_id)
{
  ...
  $ssp_fin = $ci->db->query(
    ...
    WHERE ssp_peserta_d_s_id = $d_s_id"
  )->row_array();
...
function returnNilaiCB($d_s_id, $semester)
{
  ...
  $cb_fin = $ci->db->query(
    ...
    FROM nilai_cb
    LEFT JOIN topik_cb ON nilai_cb_topik_cb_id = topik_cb_id
    WHERE nilai_cb_d_s_id = $d_s_id AND topik_cb_semester = $semester"
  )->result_array();
...
function returnRataMapelKelas($mapel_id, $kelas_id)
{
  ...
  $siswa = $ci->db->query(
    ...
    WHERE d_s_kelas_id = $kelas_id AND mapel_id = $mapel_id"
  )->row_array();
...
function returnKurangKkmKelas($mapel_id, $kelas_id, $jenis, $semester)
{
  ...
  if ($jenis == 1 && $semester == 1) {
    $siswa = $ci->db->query(
      ...
      LEFT JOIN mapel ON uj_mapel_id = mapel_id
      LEFT JOIN d_s ON d_s_id = uj_d_s_id
      WHERE d_s_kelas_id = $kelas_id AND mapel_id = $mapel_id AND uj_mid1_kog < mapel_kkm"
    )->row_array();
    ...
    $siswa = $ci->db->query(
      ...
      LEFT JOIN mapel ON uj_mapel_id = mapel_id
      LEFT JOIN d_s ON d_s_id = uj_d_s_id
      WHERE d_s_kelas_id = $kelas_id AND mapel_id = $mapel_id AND uj_mid2_kog < mapel_kkm"
    )->row_array();
    ...
    $siswa = $ci->db->query(
      ...
      LEFT JOIN mapel ON uj_mapel_id = mapel_id
      LEFT JOIN d_s ON d_s_id = uj_d_s_id
      WHERE d_s_kelas_id = $kelas_id AND mapel_id = $mapel_id AND uj_fin1_kog < mapel_kkm"
    )->row_array();
    ...
    $siswa = $ci->db->query(
      ...
      LEFT JOIN mapel ON uj_mapel_id = mapel_id
      LEFT JOIN d_s ON d_s_id = uj_d_s_id
      WHERE d_s_kelas_id = $kelas_id AND mapel_id = $mapel_id AND uj_fin2_kog < mapel_kkm"
    )->row_array();
  }
...
function returnnilSSPSiswa($d_s_id, $semester, $ssp_id)
{
  ...
  $ssp_fin = $ci->db->query(
    ...
    LEFT JOIN ssp_topik ON ssp_nilai_ssp_topik_id = ssp_topik_id
    LEFT JOIN ssp ON ssp_id = ssp_topik_ssp_id
    WHERE ssp_nilai_d_s_id = $d_s_id AND ssp_topik_semester = $semester AND ssp_id = $ssp_id"
  )->row_array();
...
function cek_absen_kegiatan($kr_id, $event_id)
{
  ...
  $det = $ci->db->query(
    ...
    FROM d_event
    WHERE d_event_event_id = $event_id AND d_event_kr_id = $kr_id"
  )->row_array();
...
function return_tamb_by_guru($kr_id, $t_id)
{
  ...
  $det = $ci->db->query(
    ...
    FROM beban_tam
    WHERE beban_tam_kr_id = $kr_id AND beban_tam_t_id = $t_id"
  )->row_array();

  return $det;
}

function return_mapel_by_kelas($kelas_id)
{
  $ci = &get_instance();
...
function detail_nil_pa($kr_penilai, $kr_dinilai, $t_id, $jabatan_kpi_id){
  $ci = &get_instance();
