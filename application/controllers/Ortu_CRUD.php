...
class Ortu_CRUD extends CI_Controller
{
  ...
  public function naik_kelas()
  {
    ...
    if (walkel_menu() >= 1) {
      ...
      $this->load->view('templates/sidebar', $data);
      $this->load->view('templates/topbar', $data);
      $this->load->view('ortu_crud/naik_kelas', $data);
      $this->load->view('templates/footer');
    } else {
      //jika bukan walkel redirect
      redirect('Profile');
  ...
  public function daftar_siswa_naik()
  {
    ...
    if ($kelas_id) {
      ...
      $data['sis_all'] = $this->db->query(
        "SELECT d_s_id, sis_nama_depan, sis_nama_bel, sis_no_induk, sis_username, d_s_naik
        FROM d_s
        LEFT JOIN sis ON d_s_sis_id = sis_id
        WHERE d_s_kelas_id = $kelas_id
        ORDER BY sis_nama_depan"
      ...
      $this->load->view('templates/sidebar', $data);
      $this->load->view('templates/topbar', $data);
      $this->load->view('ortu_crud/daftar_siswa_naik', $data);
      $this->load->view('templates/footer');
    } else {
      redirect('Profile');
    }
  ...
  public function proses_naik()
  {
    ...
    //dari method post
    $d_s_id = $this->input->post('d_s_id[]', true);
    $d_s_naik = $this->input->post('d_s_naik[]', true);
    ...
    if ($d_s_id) {
      ...
      $this->db->update_batch('d_s', $data, 'd_s_id');
      $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Update Kenaikan/Kelulusan Siswa Sukses!</div>');
      redirect('Profile');
    } else {
      redirect('Profile');
    }
