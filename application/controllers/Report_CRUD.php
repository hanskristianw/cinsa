...
class Report_CRUD extends CI_Controller
{
  ...
  public function show()
  {
    ...
    if ($this->input->post('siswa_check[]', TRUE) || $this->input->post('semester', TRUE)) {
      ...
      $data['guru_cb'] = $this->db->query(
        "SELECT kr_gelar_depan, kr_nama_depan, kr_nama_belakang, kr_gelar_belakang, kr_ttd
        FROM kelas
  ...
  public function yppi()
  {
    ...
    $this->load->view('templates/sidebar', $data);
    $this->load->view('templates/topbar', $data);
    $this->load->view('Report_CRUD/yppi', $data);
    $this->load->view('templates/footer');
  }

  public function yppi_show()
  {

    if ($this->input->post('siswa_check[]', TRUE) || $this->input->post('semester', TRUE)) {

      if (count($this->input->post('siswa_check[]', TRUE)) == 0) {
        $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Pilih 1 atau lebih siswa!</div>');
        redirect('Report_CRUD');
      }
      ...
      $this->load->view('templates/sidebar', $data);
      $this->load->view('templates/topbar', $data);
      $this->load->view('Report_CRUD/yppi_show', $data);
      $this->load->view('templates/footer');
    } else {
      $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Do not access page directly!</div>');
      redirect('Profile');
